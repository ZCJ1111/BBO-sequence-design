...
from utils.seq_utils import  sample_new_seqs,levenshtein_distance,check_cdr_constraints_all,check_cdr_constraints
...
class BO(flexs.Explorer):
    ...
    def pick_action(self, all_measured_seqs,x_central_local ,all_seqs, threshold=10):
        """Pick action."""
        ...
        for i in range(len(candidate_pool_)):
            ...
            if dist<threshold and constraint == True:
        ...
        print('states to screen', len(states_to_screen))
    ...
    def propose_sequences(
        ...
        while self.model.cost - prev_cost < self.model_queries_per_batch:
            uncertainty, new_state_string, _ = self.pick_action(all_measured_seqs,x_central_local,kwargs["all_seqs"])
